...
- `.codex/state/orchestrator-state.json`（orchestrator の repo ローカル状態台帳）

更新手段（v1）:

- `bash toolbox/skills/orchestrator-worker/scripts/update-task-state.sh init`
- `bash toolbox/skills/orchestrator-worker/scripts/update-task-state.sh upsert --task-id <id> --status <status> --owner <agent>`
- `bash toolbox/skills/orchestrator-worker/scripts/update-task-state.sh set-status --task-id <id> --status <status>`
...
- 状態更新ヘルパー: `toolbox/skills/orchestrator-worker/scripts/update-task-state.sh`
...
- 実装は repo 直下 `scripts/` に依存せず、スキル配下だけで状態更新を完結させる。
